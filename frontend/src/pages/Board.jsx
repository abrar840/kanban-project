...
const Board = ({ board, setAddMethod }) => {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`ws://127.0.0.1:8000/ws/${board_id}`);
    wsref.current = ws;
    ...
    ws.onmessage = (event) => {
      ...
      if (data.type === "task_created") {
        // Update local state when another user moves a task
        console.log("Task moved by another user:", data);

        setSavedData((prev) => ({
          ...prev,
          [data.task.position]: [...prev[data.task.position], data.task],
        }));
        ...
        setSavedData((prev) => {
          for (const pos of [1, 2, 3]) {
            ...
            if (index !== -1) {
              ...
              movedCard.position = data.task.position;
              const toList = [...prev[data.task.position], movedCard];
              return {
                ...prev,
                [pos]: from,
                [data.task.position]: toList,
              };
            }
          }
  ...
  const onDrop = async (e, toColumn) => {
    ...
    if (draggedCard) {
      ...
      if (res) {
        // alert(draggedCard.task.board_id);
              ...
              {menuOpen && (
                <div className="menu absolute top-28 right-17 w-64 bg-[#f1f2f4] p-4 rounded-lg shadow-lg z-50">
                  ...
                  <ul className="flex flex-col gap-2 mt-2">
                    <li
                      className="bg-white p-3 rounded shadow hover:bg-gray-100 transition text-red-600"
                  ...
                  {savedData[1].map((task, index) => (
                    ...
                    >
                      <div className="  shadow rounded-lg bg-white text-black p-2 flex flex-row justify-between">
                        ...
                        {task.user?.full_name && (
                          <div className="w-8 h-8 bg-[#aabeed] text-white rounded-full flex items-center justify-center text-sm font-semibold">
